...
export default function ChatPage() {
    ...
    return (
        <div className='flex lg:ml-[16%] h-screen '>
            <div className="w-full lg:w-[500px]  border-r">
                <h1 className={`font-bold text-2xl my-5 pl-3 ${(displayWidth < 1024 && selectedParticipant) && 'hidden'}`}>{user?.username}</h1>
                <div className='flex px-3 justify-between'>
                    <span className={`font-medium text-lg ${(displayWidth < 1024 && selectedParticipant) && 'hidden'}`}>Messages</span>
                </div>
                {
                    (!selectedParticipant || displayWidth >= 1024) ?
                        ...
                        : <div className='flex flex-col justify-between h-[99vh]'>
                            <div onClick={()=>navigate(`/profile/${selectedParticipant?._id}`)} className='flex items-center gap-3 px-3 py-2 border-b'>
                                <Avatar className="size-10">
                                    <AvatarImage src={selectedParticipant.profilePicture} />
                                    <AvatarFallback>CN</AvatarFallback>
                                </Avatar>

                                <span className='text-lg font-medium'>{selectedParticipant.username}</span>
                                {
                                    onlineUsers.includes(selectedParticipant?._id) && <span className='size-3 rounded-full bg-green-600 absolute top-9 left-10'></span>
                                }
